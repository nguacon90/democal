...
import {ViewTypes, DATE_FORMAT, DATETIME_FORMAT, DemoData} from './index'
const RESOURCES_DATA = 'RESOURCES_DATA';
...
export default class SchedulerData {
    ...
    getResources() {
        let resources = localStorage.getItem(RESOURCES_DATA);
        if(resources == null) {
            localStorage.setItem(RESOURCES_DATA, JSON.stringify(DemoData.resources));
    ...
    getEvents() {
        let events = localStorage.getItem(EVENTS_DATA);
        if(events == null) {
            localStorage.setItem(EVENTS_DATA, JSON.stringify(DemoData.events));
    ...
    // setBookingResources(bookingResources) {
    //     this._validateResource(bookingResources);
    //     this.bookingResources = Array.from(new Set(bookingResources));
    //     this._createRenderData();
    ...
    moveEvent(event, newSlotId, newSlotName, newStart, newEnd){
        ...
        if(this._isBookingEvent(event)) {
            ...
            cloneEvent.groupName = newSlotName;
            cloneEvent.resourceId = newSlotId;
    ...
    _removeEvents(event) {
        this._enableResizeAndMoveBookingEventBy(event);
        let pos = this.events.indexOf(event);
        if(pos > -1) {
            this.events.splice(pos, 1);
            this._saveEvents(this.events);
        }
    }
    _enableResizeAndMoveBookingEventBy(event) {
        let self = this;
